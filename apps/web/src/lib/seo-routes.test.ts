...
describe("crawl controls", () => {
  it("includes public research directories but excludes query search", async () => { const urls = (await sitemap()).map((entry) => entry.url); expect(urls).toContain("https://www.forexmax.com/"); expect(urls).toContain("https://www.forexmax.com/markets"); expect(urls).toContain("https://www.forexmax.com/macro"); expect(urls).not.toContain("https://www.forexmax.com/search"); });
