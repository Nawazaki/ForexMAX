...
describe("crawl controls", () => {
  it("includes the public home route but excludes query search", async () => { const urls = (await sitemap()).map((entry) => entry.url); expect(urls).toContain("https://www.forexmax.com/"); expect(urls).not.toContain("https://www.forexmax.com/search"); });
