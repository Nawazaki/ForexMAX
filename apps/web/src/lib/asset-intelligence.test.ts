...
describe("asset intelligence profiles", () => {
  ...
  it("does not present an inactive gold price as a live input", () => {
    expect(getAssetIntelligenceProfile("gold-macro-drivers")?.overview).toContain("no active eligible gold-price series");
  });
  ...
  it("registers the five priority assets with linked observation dependencies", () => {
    ["gold-macro-drivers", "eurusd", "usdjpy", "us-10y", "sp500"].forEach((slug) => {
      expect(getAssetIntelligenceProfile(slug)?.tapeIds.length).toBeGreaterThan(0);
    });
  });
});
