...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; const record = await getPublishedPage(slug); return record ? buildMetadata({ title: record.title, description: record.description, path: record.route, image: record.ogImage }) : {}; }
export default async function StaticPage({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; if (reserved.has(slug)) notFound(); const record = await getPublishedPage(slug); if (!record) notFound(); const schema = { "@context": "https://schema.org", "@type": "WebPage", name: record.title, description: record.description, url: `https://forexmax.com${record.route}` }; return <PageShell><article className="shell article static-page"><script type="application/ld+json" dangerouslySetInnerHTML={jsonLd(schema)} /><p className="eyebrow">FOREXMAX RESEARCH</p><h1>{record.title}</h1><p className="article-deck">{record.description}</p><ArticleBody html={record.contentHtml} /></article></PageShell>; }
