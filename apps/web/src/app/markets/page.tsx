...
import { assetIntelligenceProfiles } from "@/lib/asset-intelligence";
...
export default function MarketsPage() {
  ...
  return <PageShell><script type="application/ld+json" dangerouslySetInnerHTML={jsonLd(schema)} /><div className={styles.page}>
    ...
    <section className={`shell ${styles.assetContexts}`}><div><p className={styles.kicker}>SOURCE-AWARE ASSET CONTEXT</p><h2>Start with the published record.</h2><p>These pages contain named observations and evidence requirements. They are not price-target or trade-signal pages.</p></div><div>{assetIntelligenceProfiles.map((asset) => <article key={asset.slug}><p>{asset.eyebrow}</p><h3><Link href={`/markets/assets/${asset.slug}`}>{asset.title}</Link></h3><span>{asset.description}</span></article>)}</div></section>
