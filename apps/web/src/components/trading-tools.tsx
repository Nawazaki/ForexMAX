...
export function TradingTools() {
  ...
  const positionIsValid = useMemo(() => hasValidPositionInputs(balance, risk, stopPips, pipValue), [balance, risk, stopPips, pipValue]);
  const rewardIsValid = useMemo(() => hasValidRiskRewardInputs(entry, stop, target), [entry, stop, target]);
  const f = (setter: (value: string) => void) => (event: ChangeEvent<HTMLInputElement>) => setter(event.target.value);
  return <div className="tools-grid"><section className="tool-card"><p className="eyebrow">POSITION SIZE</p><h2>Define the trade before the risk.</h2><div className="field-grid"><label>Account balance<input inputMode="decimal" value={balance} onChange={f(setBalance)} /></label><label>Risk %<input inputMode="decimal" value={risk} onChange={f(setRisk)} /></label><label>Stop (pips)<input inputMode="decimal" value={stopPips} onChange={f(setStopPips)} /></label><label>Pip value / unit<input inputMode="decimal" value={pipValue} onChange={f(setPipValue)} /></label></div><output aria-live="polite">{positionIsValid ? <>Risk: <strong>${position.money.toFixed(2)}</strong><br />Approx. units: <strong>{Math.round(position.units).toLocaleString()}</strong></> : "Enter positive numeric values for balance, risk, stop and pip value to calculate."}</output></section><section className="tool-card"><p className="eyebrow">RISK / REWARD</p><h2>Check asymmetry before entry.</h2><div className="field-grid"><label>Entry<input inputMode="decimal" value={entry} onChange={f(setEntry)} /></label><label>Stop<input inputMode="decimal" value={stop} onChange={f(setStop)} /></label><label>Target<input inputMode="decimal" value={target} onChange={f(setTarget)} /></label></div><output aria-live="polite">{rewardIsValid ? <>Risk / reward: <strong>1:{reward.ratio.toFixed(2)}</strong><br />Break-even before costs: <strong>{reward.breakEvenPercent.toFixed(1)}%</strong></> : "Enter numeric prices and a stop that differs from entry to calculate."}</output></section><section className="tool-card session-clock"><p className="eyebrow">SESSION CLOCK</p><h2>Use time as context, not a signal.</h2><time suppressHydrationWarning dateTime={now.toISOString()}>{formatUtcClock(now)}</time><p>Session labels are indicative. Verify holidays, daylight saving and provider-specific trading hours.</p></section></div>;
}
